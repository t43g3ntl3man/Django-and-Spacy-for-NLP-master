...
def submit(request):
    ...
    if request.method=="POST":
        ...
        if 1:
            ...
            xz_data = gz_data
            gz_data = gz_data.toprettyxml()
            driver = webdriver.Firefox(executable_path=r'/home/k0nc3pt/Desktop/geckodriver', options=options)
        ...
        #   Code for Nouns extraction using Spacy
            ...
            return render(request,'index.html',{'sentence':request.POST.get('sentence') ,'pos':zip(nouns,spans),'nouns':nouns, 'verbs': spans,'value':value,'condition':condition, 'flag':flag,'button':"Convert To SBVR", 'process': process,'sentence_check': sentence_check,'classnames':finallist,'associations':associations,'notmatch':notmatch,'customized_sentence':customized_sentence,'attributes':all_attribs,'attribs':not_all_attribs})
...
def ocl(request):
    ...
    print(type(sixx))
    res.pop(0)
    jigsaw = zip(res, iinvariant , vals, atr)
    ...
    # rendering the output with all the values passed as context
    return render(request,'index.html',{'sentence': request.GET.get('sentence') ,'button':"Try A New Sentence?", 'customized_sentence': request.GET.get('customized_sentence'),'condition':iinvariant,'value':vals,'classnames': ast.literal_eval(request.GET.get('classnames')), 'jigsaw': jigsaw, 'ranger': range(2)})
